/* ============================================================
   ...
   resulting public URL is written to the profiles table — that
   URL works for anyone who loads it, on any device, which is what
   ...
   without waiting on a Supabase round trip. Supabase stays the
   cross-device source of truth; the bridge is just a fast local
   cache of it.

   IMPORTANT: the actual database write is awaited and checked now.
   It used to be fire-and-forget (saveProfile(...).catch(console.
   error)), which meant a failed save — wrong Supabase URL, a
   missing RLS policy, a missing table/column, a missing "avatars"
   bucket, etc. — was swallowed silently: the page would still
   redirect as if everything worked, with nothing actually saved.
   Now a failure shows a specific, readable message right on the
   form and the person stays there to retry, instead of an
   unexplained popup (or worse, no feedback at all) followed by
   data that quietly never saved.
   ============================================================ */
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Photo upload/storage IS wired up (Supabase Storage) — this
  // fallback only kicks in for someone who hasn't picked a photo at
  // all: their Google account photo, or a generated initial.
  ...
  // A single inline banner for save failures, injected here rather
  // than requiring an edit to the HTML/CSS — this file works as a
  // drop-in replacement on its own. Sits just above the Save button.
  let formError = form ? form.querySelector('#profileFormError') : null;
  if (form && !formError) {
    formError = document.createElement('p');
    formError.id = 'profileFormError';
    formError.hidden = true;
    Object.assign(formError.style, {
      marginTop: '-6px',
      marginBottom: '18px',
      padding: '12px 14px',
      fontSize: '13.5px',
    ...
    if (submitBtn && submitBtn.parentNode === form) {
      form.insertBefore(formError, submitBtn);
    } else {
  ...
  // away, and refresh the local bridge from Supabase so every other
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    // Avatar upload first, if a new one was picked. A failure here
    // shouldn't block the rest of the profile from saving, so it just
    // falls back to whatever photo (if any) already existed — but it
    // IS reported now instead of failing silently.
    if (pendingAvatarFile) {
      ...
      try {
        ...
        showFormError(
          'Your photo couldn\u2019t be uploaded (' +
          (auth.friendlyError ? auth.friendlyError(err) : 'unknown error') +
          '). The rest of your profile will still be saved below.'
        );
      }
    ...
    // The actual save — awaited and checked, so a failure here is
    // never silently ignored or mistaken for success.
    ...
    try {
      ...
      showFormError(auth.friendlyError ? auth.friendlyError(err) : 'Could not save your profile — please try again.');
      return; // stop here — nothing was actually saved, so don't act like it was
    }
